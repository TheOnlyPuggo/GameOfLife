...
inline std::ostream& operator<<(std::ostream& output, const GridPos& input)
{
    output << "GridPos(" << input.x << ", " << input.y << ')';
    return output;
}
...
inline GridPos operator+(const GridPos& gridPos1, const GridPos& gridPos2)
{
    return GridPos{ gridPos1.x + gridPos2.x, gridPos1.y + gridPos2.y };
}
